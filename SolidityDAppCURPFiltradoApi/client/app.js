App = {
  ...
  renderTasks: async () => {
    ...
    for (let i = 1; i <= taskCounterNumber; i++) {
      ...
      const _curp = task[7];
      const _direccionCuenta = task[8]
      const taskDone = task[9];
      const taskCreatedAt = task[10];
      ...
      let taskElement = `<div class="card bg-dark rounded-0 mb-2">
        ...
        <div id = "myCards"class="card-body">
          <span>${"Datos de consulta"}</span></br>
          <span>Nombres: ${_nombres}</span></br>
          <span>Apellido Paterno: ${_apaterno}</span></br>
          <span>Apellido Materno: ${_amaterno}</span></br>
          <span>Fecha de nacimiento: ${_fechaNacimiento}</span></br>
          <span>Género: ${_genero}</span></br>
          <span>Estado: ${_estado}</span></br>
          <p class="text-muted">Creado el ${new Date(
            taskCreatedAt * 1000
          ).toLocaleString()}</p>
          </label>
          <p class="text-muted">Creado por ${_direccionCuenta}</p>
          </label>
        </div>
      </div>`;
      //Validación
      if(App.account.toUpperCase() == _direccionCuenta.toUpperCase())
      {
        html += taskElement;
      }
    }
  ...
  createCurp: async (nombres, apaterno, amaterno, fechaNacimiento, genero, estado, curp) => {
    try {
      const result = await App.tasksContract.createTask(nombres, apaterno, amaterno, fechaNacimiento, genero, estado, curp,{
      //createTask(title, description, {
        from: App.account,
      });
      console.log(result.logs[0].args);
